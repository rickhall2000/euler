...
(defn get-val [point]
  (let [a (first point) b (last point)]
    (if (and (pos? a) (pos? b)
             (> grid-size a) (> grid-size b))
      ((box b) a) 0)))

(defn get-vals [points]
  (map get-val points))

(defn get-all-lines-from-point [[a b]]
  ((juxt hl vl rdl ldl) [a b]))

(defn best-from-point [[a b]]
  (apply max
         (map #(reduce * %)
              (map get-vals
...
(defn handle-tens [n]
  (let [val (rem n 100)]
    (cond
      (< val 10) (digit-to-word (first (seq (str val))))
      (< val 20) (teen-to-word val)
      :else (let [strs (seq (str val))]
              (str (ten-to-word (first strs))
                   (digit-to-word (first (rest strs))))))))
...
(defn num-to-words [n]
  (let [digs (seq (str n))]
    (cond
      ...
      (< n 20) (teen-to-word n)
      (< n 100)  (handle-tens n)
      (< n 1000) (str (digit-to-word (first digs))
                      "hundredand"
                      (handle-tens n))
      (= n 1000) "onethousand"
      :else "")))
